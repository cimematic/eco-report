...
const firebaseConfig = {
  apiKey: "AIzaSyBHQi78XgsIa5p3Trh2phB80hjVsD49JEI",
  authDomain: "report2-65248.firebaseapp.com",
  projectId: "report2-65248",
  storageBucket: "report2-65248.firebasestorage.app",
  messagingSenderId: "689995627940",
  appId: "1:689995627940:web:55a619a9d5eed6e6485531"
};
...
const foodItems = [
  ...
  { productName: '즉석밥 白반 210g x3', brand: 'Emart24', price: 3600, storage: '상온' },
...
async function main() {
  try {
    console.log('🔥 초기 데이터 업로드를 시작합니다...\n');

    const { reports, foodShares } = generateSeedData();

    console.log(`📋 생성된 데이터: ${reports.length}개 리포트, ${foodShares.length}개 푸드쉐어\n`);
    ...
    for (let i = 0; i < reports.length; i++) {
      ...
      console.log(`  ✅ 리포트 ${i + 1}/${reports.length} 추가됨 → ${docRef.id}`);
    }

    console.log('');

    // Upload foodShares
    const foodSharesRef = collection(db, 'foodShares');
    for (let i = 0; i < foodShares.length; i++) {
      const share = foodShares[i];
      const data = Object.fromEntries(
        Object.entries({ ...share, createdAt: Timestamp.now() }).filter(
          ([, v]) => v !== undefined
        )
      );
      const docRef = await addDoc(foodSharesRef, data);
      console.log(`  ✅ 푸드쉐어 ${i + 1}/${foodShares.length} 추가됨 → ${docRef.id}`);
    }

    console.log('\n✅ 모든 시드 데이터 업로드가 완료되었습니다!');
    process.exit(0);
  } catch (error) {
    console.error('\n❌ 오류 발생:', error);
    process.exit(1);
